...
- Start command: `python manage.py migrate && python manage.py sync_repo_media && python manage.py seed_catalog_if_empty && gunicorn config.wsgi:application`
- Persistent disk is used for SQLite and uploaded media

Because Render runs `python manage.py migrate` during service startup, newly added data/schema migrations are applied automatically on the deployed environment as part of the next deploy or restart. The startup flow now seeds the catalog only when the database is empty, which avoids overwriting live product data or clearing generated product codes on later restarts.
