(function () {
    ...
    const browserStepMessages = {
        ...
        cake_order: 'Opening the cake customization page with the latest review-first checkout flow...',
        cake_tracking: 'Showing the demo cake order tracking page...',
        packages: 'Showing the live package catalog...',
        package_order: 'Opening the package order flow with manual GCash proof and order review...',
    ...
    function chooseRadioOption(name, options = {}) {
        const inputs = Array.from(document.querySelectorAll(`input[name="${name}"]`));
        if (!inputs.length) {
            return false;
        }

        const normalizedNeedles = Array.isArray(options.labelIncludes)
            ? options.labelIncludes.map((value) => String(value || '').toLowerCase()).filter(Boolean)
            : [];

        let target = null;
        if (normalizedNeedles.length) {
            target = inputs.find((input) => {
                const haystack = `${input.dataset.label || ''} ${input.value || ''}`.toLowerCase();
                return normalizedNeedles.some((needle) => haystack.includes(needle));
            });
        }

        if (!target && Number.isInteger(options.preferIndex) && inputs[options.preferIndex]) {
            target = inputs[options.preferIndex];
        }

        if (!target && options.preferNonEmpty) {
            target = inputs.find((input) => String(input.value || '').trim() !== '');
        }
    ...
    function rerunBrowserDemoStep(plan, delay) {
        scheduleBrowserDemoAction(() => {
            const activePlan = loadBrowserDemoPlan();
            if (activePlan) {
                maybeRunBrowserDemo();
            }
        }, delay);
    ...
    function fillCakeOrderForm(plan) {
        ...
        chooseRadioOption('size', { preferIndex: 1 });
        chooseRadioOption('shape', { labelIncludes: ['round'] });
        chooseRadioOption('flavor', { labelIncludes: ['chocolate'], preferIndex: 0 });
        chooseRadioOption('frosting', { labelIncludes: ['buttercream'], preferIndex: 0 });
        chooseRadioOption('filling', { labelIncludes: ['chocolate'], preferIndex: 1, preferNonEmpty: true });
        ...
        setFieldValue('input[name="delivery_street_address"]', '123 Demo Street');
        setFieldValue('input[name="delivery_barangay"]', 'Poblacion 1');
        setSelectValue('select[name="delivery_city"]', 'Lucena City');
        setFieldValue('input[name="delivery_landmark"]', 'Near City Hall');
        ...
        plan.pendingCakeOrder = 'style-review';
        saveBrowserDemoPlan(plan);
        rerunBrowserDemoStep(plan, holdDurationForPlan(plan, 1400));
    }

    function handleCakeOrderStep(plan) {
        if (plan.pendingCakeOrder === 'submitted' && isTrackingPageForType('cake')) {
        ...
        if (plan.pendingCakeOrder === 'style-review') {
            if (!clickElement('#cake-style-review-button')) {
                setStatus('Unable to find the cake design review button for the browser demo.', 'error');
                return true;
            }
            plan.pendingCakeOrder = (plan.payment_mode || 'gcash') === 'gcash' ? 'payment-proof-open' : 'order-review';
            saveBrowserDemoPlan(plan);
            rerunBrowserDemoStep(plan, holdDurationForPlan(plan, 1200));
            return true;
        }

        if (plan.pendingCakeOrder === 'payment-proof-open') {
            if (!clickElement('#continueToPaymentProof')) {
                setStatus('Unable to open the cake payment proof step for the browser demo.', 'error');
                return true;
            }
            plan.pendingCakeOrder = 'payment-proof-fill';
            saveBrowserDemoPlan(plan);
            rerunBrowserDemoStep(plan, holdDurationForPlan(plan, 1200));
            return true;
        }

        if (plan.pendingCakeOrder === 'payment-proof-fill') {
            setFieldValue('input[name="reference_number"]', `CAKE-DEMO-${Date.now()}`);
            assignFileToInput(document.querySelector('input[name="proof_image"]'), 'cake-proof-demo.png');
            plan.pendingCakeOrder = 'payment-proof-review';
            saveBrowserDemoPlan(plan);
            rerunBrowserDemoStep(plan, holdDurationForPlan(plan, 1200));
            return true;
        }

        if (plan.pendingCakeOrder === 'payment-proof-review') {
            if (!clickElement('#continueToOrderReview')) {
                setStatus('Unable to continue from cake payment proof to order review in the browser demo.', 'error');
                return true;
            }
            plan.pendingCakeOrder = 'order-review';
            saveBrowserDemoPlan(plan);
            rerunBrowserDemoStep(plan, holdDurationForPlan(plan, 1200));
            return true;
        }

        if (plan.pendingCakeOrder === 'order-review') {
            if (!clickElement('#cake-review-button')) {
                setStatus('Unable to find the cake order review button for the browser demo.', 'error');
                return true;
            }
            plan.pendingCakeOrder = 'confirm';
            saveBrowserDemoPlan(plan);
            rerunBrowserDemoStep(plan, holdDurationForPlan(plan, 1200));
            return true;
        }

        if (plan.pendingCakeOrder === 'confirm') {
            const form = document.getElementById('cake-order-form');
            const confirmButton = document.getElementById('cake-confirm-button');
            if (!form || !confirmButton) {
                setStatus('Unable to confirm the cake order in the browser demo.', 'error');
                return true;
            }
            plan.pendingCakeOrder = 'submitted';
            saveBrowserDemoPlan(plan);
            scheduleFormSubmission(() => {
                confirmButton.click();
            }, holdDurationForPlan(plan, 1300));
            return true;
        }
    ...
    function fillPackageStepTwo(plan) {
        ...
        setSelectValue('select[name="theme"]', 'Birthday');
        chooseRadioOption('cake_size', { preferIndex: 1 });
        chooseRadioOption('shape', { labelIncludes: ['round'] });
        chooseRadioOption('flavor', { labelIncludes: ['chocolate'], preferIndex: 0 });
        chooseRadioOption('frosting', { labelIncludes: ['buttercream'], preferIndex: 0 });
        chooseRadioOption('filling', { labelIncludes: ['chocolate'], preferIndex: 1, preferNonEmpty: true });
    ...
    function fillPackageStepThree(plan) {
        ...
        setFieldValue('input[name="contact_phone"]', '09171234567');
        setFieldValue('input[name="contact_email"]', 'paneldemo@example.com');
        assignFileToInput(document.querySelector('input[name="design_reference"]'), 'package-design-demo.png');
        ...
        plan.pendingPackageOrder = (plan.payment_mode || 'gcash') === 'gcash' ? 'step-three-payment-proof-open' : 'step-three-order-review';
        saveBrowserDemoPlan(plan);
        rerunBrowserDemoStep(plan, holdDurationForPlan(plan, 1400));
    }
    ...
    function handlePackageOrderStep(plan) {
        ...
        if (window.location.pathname === '/package-payment/') {
            ...
            if (plan.pendingPackageOrder === 'step-three-payment-proof-open') {
                if (!clickElement('#packageContinueToPaymentProof')) {
                    setStatus('Unable to open the package payment proof step for the browser demo.', 'error');
                    return true;
                }
                plan.pendingPackageOrder = 'step-three-payment-proof-fill';
                saveBrowserDemoPlan(plan);
                rerunBrowserDemoStep(plan, holdDurationForPlan(plan, 1200));
                return true;
            }

            if (plan.pendingPackageOrder === 'step-three-payment-proof-fill') {
                ...
                plan.pendingPackageOrder = 'step-three-payment-proof-review';
                saveBrowserDemoPlan(plan);
                rerunBrowserDemoStep(plan, holdDurationForPlan(plan, 1200));
                return true;
            }

            if (plan.pendingPackageOrder === 'step-three-payment-proof-review') {
                if (!clickElement('#packageContinueToOrderReview')) {
                    setStatus('Unable to continue from package payment proof to order review in the browser demo.', 'error');
                    return true;
                }
                plan.pendingPackageOrder = 'step-three-order-review';
                saveBrowserDemoPlan(plan);
                rerunBrowserDemoStep(plan, holdDurationForPlan(plan, 1200));
                return true;
            }

            if (plan.pendingPackageOrder === 'step-three-order-review') {
                if (!clickElement('#package-review-button')) {
                    setStatus('Unable to find the package order review button for the browser demo.', 'error');
                    return true;
                }
                plan.pendingPackageOrder = 'step-three-confirm';
                saveBrowserDemoPlan(plan);
                rerunBrowserDemoStep(plan, holdDurationForPlan(plan, 1200));
                return true;
            }

            if (plan.pendingPackageOrder === 'step-three-confirm') {
                const confirmButton = document.getElementById('package-confirm-button');
                if (!confirmButton) {
                    setStatus('Unable to confirm the package order in the browser demo.', 'error');
                    return true;
                }
